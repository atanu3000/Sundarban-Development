            ...
            <div class="max-w-sm bg-white rounded-lg hover:scale-110 hover:shadow-2xl transition duration-300 ease-in-out">
                ...
                <div class="p-5">
                    ...
                    if (mysqli_num_rows($check_to_cart) == 0) { ?>
                        <a <?php if (isset($_SESSION['cid'])) { ?>
                                href="add_to_cart.php?pid=<?php echo $row['id'] ?>&cid=<?php echo $_SESSION['cid'] ?>" <?php } else { ?>
                                href='./client/login.php' onclick="alert('Login to access ADD TO CART');" <?php } ?>
                    ...
                    <?php } else { ?>
                        <a href="cart.php"
                            class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-yellow-500 rounded-lg uppercase mt-2 hover:bg-orange-400">
                            <svg class="w-5 h-5 mr-2 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 19 20">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 15a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm0 0h8m-8 0-1-4m9 4a2 2 0 1 0 0 4 2 2 0 0 0 0-4Zm1-4H5m0 0L3 4m0 0h5.501M3 4l-.792-3H1m11 3h6m-3 3V1" />
